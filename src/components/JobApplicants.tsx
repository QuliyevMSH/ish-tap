...
const JobApplicants: React.FC<JobApplicantsProps> = ({
  ...
  if (showAllApplicants && isOwner) {
    return <Dialog open={showAllApplicants} onOpenChange={onOpenChange}>
        <DialogContent>
          <DialogHeader>
            <DialogTitle>İşi istəyənlərin siyahısı</DialogTitle>
          </DialogHeader>
          <div className="space-y-4 mt-4">
            {applicants.map(profile => <div key={profile.id} className="flex items-center space-x-4 p-2 hover:bg-gray-100 rounded-md cursor-pointer" onClick={() => handleProfileClick(profile.id)}>
                <Avatar>
  ...
  return <div className="flex items-center space-x-2">
      <div className="flex -space-x-2">
        {visibleApplicants.map(profile => <Avatar key={profile.id} className="border-2 border-white">
            <AvatarImage src={getAvatarUrl(profile)} />
            <AvatarFallback>{profile.name?.[0]}{profile.surname?.[0]}</AvatarFallback>
          </Avatar>)}
      </div>
      {remainingCount > 0 && <span className="text-sm dark:color-white-500">+{remainingCount} nəfər</span>}
    </div>;
};
